...
namespace Jelewry_Store.Controllers
{
    ...
    public class BranchController : ControllerBase
    {
        ...
        [HttpPost]
        public ActionResult<bool> Post([FromBody] Branch branch)
        {
        ...
        [HttpPut("{id}")]
        public ActionResult<bool> Put(int codeBranch, [FromBody] Branch branch)
        {
            if (codeBranch < 0) return BadRequest();
        ...
        [HttpDelete("{id}")]
        public ActionResult<bool> Delete(int codeBranch)
        {
            if (codeBranch < 0) return BadRequest();
            Branch s = branchServices.GetBranchByCode(codeBranch);
            if (s == null)
